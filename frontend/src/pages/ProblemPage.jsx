...
const ProblemPage = () => {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      ...
      try {
        ...
        console.log('Complete problem data:', response.data);

        const startCode = response.data.startCode || [];
        let initialCode = startCode.find(sc => sc.language === langMap[selectedLanguage])?.initialCode;

        // If requested language not found, use first available language
        if (!initialCode && startCode.length > 0) {
          initialCode = startCode[0].initialCode;
          const availableLanguage = startCode[0].language.toLowerCase();
          setSelectedLanguage(availableLanguage === 'c++' ? 'cpp' : availableLanguage);
        }
  ...
  useEffect(() => {
    if (problem) {
      ...
      const initialCode = startCode.find(sc => sc.language === langMap[selectedLanguage])?.initialCode;
      if (initialCode) {
        setCode(initialCode);
      }
    }
  ...
  return (
    <div className="h-screen flex bg-base-100">
      {/* Left Panel */}
      <div className="w-3/5 flex flex-col border-r border-base-300">
        {/* Left Tabs */}
                    ...
                    <div className="space-y-4">
                      {problem.visibleTestCases.map((example, index) => (
                        <div key={index} className="bg-base-200 p-4 rounded-lg border border-base-300">
                          <h4 className="font-semibold mb-2 text-base-content">Example {index + 1}:</h4>
                          <div className="space-y-2 text-sm font-mono">
                            <div><strong className="text-base-content">Input:</strong>
                              <span className="text-base-content"> {example.input}</span>
                            </div>
                            <div><strong className="text-base-content">Output:</strong>
                              <span className="text-base-content"> {example.output}</span>
                            </div>
                            <div><strong className="text-base-content">Explanation:</strong>
              ...
              <div className="flex justify-between items-center p-4 border-b border-base-300">
                <div className="flex gap-2">
                  {['cpp', 'java', 'javascript'].map((lang) => {
                    const isAvailable = problem?.startCode?.some(sc =>
                      sc.language === langMap[lang]
                    );

                    return (
                      <button
                        key={lang}
                        className={`btn btn-sm ${selectedLanguage === lang ? 'btn-primary' : 'btn-ghost'} ${!isAvailable ? 'btn-disabled' : ''
                          }`}
